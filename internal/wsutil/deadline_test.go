...
// without depending on hertz-contrib/websocket internals.
...
func TestIsControlWriteContention(t *testing.T) {
	...
	// The hertz-contrib/websocket library returns errWriteTimeout (a
	...
	t.Run("hertz_write_lock_timeout_is_contention", func(t *testing.T) {
		err := &fakeNetError{msg: "websocket: write timeout", timeout: true}
		if !IsControlWriteContention(err) {
			t.Fatal("hertz write-lock timeout must be classified as contention")
		}
	})

	t.Run("hertz_write_lock_timeout_wrapped", func(t *testing.T) {
		inner := &fakeNetError{msg: "websocket: write timeout", timeout: true}
		err := fmt.Errorf("ping write: %w", inner)
		if !IsControlWriteContention(err) {
			t.Fatal("wrapped hertz write-lock timeout must still be classified as contention")
		}
	})
	...
	t.Run("generic_timeout_message_is_not_contention", func(t *testing.T) {
		// Anything that is Timeout() but does not carry the hertz lock-wait
	...
	t.Run("timeout_message_with_hertz_prefix_is_not_contention", func(t *testing.T) {
	...
	t.Run("timeout_message_with_hertz_suffix_is_not_contention", func(t *testing.T) {
