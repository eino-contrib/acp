...
//
// The body is generated first into g.buf: every generation function records its
// own import needs (g.needJSON / g.needFmt / g.needHasKey) as it emits code, so
// the import block is computed from actual usage rather than a separate pre-scan
// that could drift out of sync with the emitters. The header and import block
// are assembled afterwards and prepended to the body.
func (g *Generator) Generate(pkg string) ([]byte, error) {
	...
	// Assemble header + imports (computed from the import needs recorded during
	// body generation) and prepend them to the generated body.
	var out strings.Builder
	out.WriteString("// Code generated by cmd/generate; DO NOT EDIT.\n\n")
